...
public class CoraDigestUtilsTest {

	CoraDigestUtils digestor;

	@BeforeMethod
	private void beforeMethod() {
		digestor = new CoraDigestUtilsImp();
	}
	...
	public void testSha256Hex() {
		...
		String hashedString = digestor.sha256Hex("someString");
		assertEquals(hashedString, someStringHasehd);

		String hashedOtherString = digestor.sha256Hex("someOtherString");
	...
	public void testOnlForTestSetSha256Algorithm() {
		...
		((CoraDigestUtilsImp) digestor).onlyForTestSetSha256Algorithm(nonExistingAlgorithm);

		try {
			digestor.sha256Hex("someString");
